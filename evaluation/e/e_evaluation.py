...
Computes the evaluation metric suite:
  ...
  System-level : ACS_noev, ACS_strict, Consistency

ACS_oracle requires a separate oracle-retrieval experiment and is not
...
# directly (`python evaluation/e/e_evaluation.py`, the self-test below).
# ---------------------------------------------------------------------------
_THIS_DIR = os.path.dirname(os.path.abspath(__file__))          # .../evaluation/e
_EVAL_DIR = os.path.dirname(_THIS_DIR)                           # .../evaluation
_ECS_SCORING_DIR = os.path.join(_EVAL_DIR, "scoring")
...
class ModuleE_Evaluator:
    ...
    def run(
        ...
        from the candidate pool (text+table+def).  When provided, ACS_oracle is
        computed as ACS_strict under hypothetical perfect retrieval.
        """
    ...
    def _evaluate_instance(
        ...
        res["acs_noev"]   = ecs_metric.no_evidence_closure(v_passed)
        res["acs_strict"] = ecs_metric.strict_closure(v_passed, best_recall)

        # ACS_oracle: ACS_strict under hypothetical perfect retrieval
        oracle_recall = ev.get("oracle_recall")
        res["acs_oracle"] = ecs_metric.oracle_closure(v_passed, oracle_recall)

        # Failure reason for ACS_strict=0 instances
        if res["parse_ok"] == 0.0:
            res["failure_reason"] = "parse_fail"
        elif res["acs_strict"] == 1.0:
        ...
        logger.debug(
            ...
            f"acs_noev={res['acs_noev']:.1f} acs_strict={res['acs_strict']:.1f}"
        )
        return res
    ...
    def _finalize_report(df: pd.DataFrame) -> Dict:
        ...
        def _pool_breakdown() -> Dict:
            """ACS_strict + primary pool Hit@K, grouped by gold_primary_pool."""
            ...
            for pool in ("text", "table_cell", "definition"):
                ...
                acs_v = sub["acs_strict"].astype(float).mean(skipna=True)
                entry["ACS_strict"] = float(acs_v) if not np.isnan(acs_v) else None
        ...
        def _failure_dist() -> Dict:
            """Failure reason breakdown among ACS_strict=0 instances."""
            if "failure_reason" not in df.columns or "acs_strict" not in df.columns:
                return {}
            fail_df = df[df["acs_strict"].astype(float) == 0.0]
            if len(fail_df) == 0:
                return {}
            total = len(fail_df)
            return {
        ...
        return {
            "summary": {
                ...
                "ACS_noev":    col_mean("acs_noev"),
                "ACS_strict":  col_mean("acs_strict"),
                "ACS_oracle":  col_mean("acs_oracle"),  # None if oracle_candidates not provided
...
if __name__ == "__main__":
    ...
    expected = {"Parseable": 1.0, "F1_role": 1.0, "F1_fields": 1.0,
                ...
                "ACS_noev": 1.0, "ACS_strict": 1.0, "Consistency": 1.0}
